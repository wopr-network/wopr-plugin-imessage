...
import type { JsonRpcRequest, JsonRpcResponse, JsonRpcNotification, IncomingMessage } from "./types.js";
...
export class IMessageClient {
  ...
  private handleLine(line: string): void {
    ...
    try {
      ...
      logger.warn({ msg: "Failed to parse imsg JSON", line: trimmed.substring(0, 200) });
      return;
    }
  ...
  private handleNotification(notification: JsonRpcNotification): void {
    logger.debug({ msg: "imsg notification", method: notification.method, params: notification.params });
    ...
    switch (notification.method) {
      ...
      case "message.received":
        ...
        break;
      case "error":
        logger.error({ msg: "imsg error notification", error: notification.params });
        this.onError?.(new Error(String(notification.params?.message || "Unknown imsg error")));
        break;
      default:
        logger.debug({ msg: "Unknown imsg notification", method: notification.method });
    }
  }
