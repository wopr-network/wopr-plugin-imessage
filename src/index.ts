...
async function sendResponse(originalMsg: IncomingMessage, text: string, config: IMessageConfig) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      ...
      logger.debug({ msg: "Sent iMessage response", chat_id: originalMsg.chat_id });
      ...
      logger.error({ msg: "Failed to send iMessage response", error: error.message });
    }
  }
}
...
const plugin: WOPRPlugin = {
  ...
  async init(context: WOPRPluginContext) {
    ...
    // Get config
    const fullConfig = ctx.getConfig<{ channels?: { imessage?: IMessageConfig } }>();
    ...
    try {
      ...
      logger.error({ msg: "Failed to initialize iMessage client", error: error.message });
      throw error;
    }
  },
