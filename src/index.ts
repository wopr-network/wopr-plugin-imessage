...
const configSchema: ConfigSchema = {
	...
	description:
		"iMessage/SMS integration via imsg CLI. Requires macOS with Messages.app.",
	fields: [
		{
			name: "enabled",
			type: "checkbox",
...
const plugin: WOPRPlugin = {
	...
	async init(context: WOPRPluginContext) {
		...
		if (!isMacOS()) {
			logger.warn(
				"iMessage plugin requires macOS. Current platform: " + process.platform,
			);
			return;
		}
			...
			client = new IMessageClient({
				...
				onMessage: (msg) => {
					...
					const freshConfig: IMessageConfig =
						ctx!.getConfig<{ channels?: { imessage?: IMessageConfig } }>()
							?.channels?.imessage || {};
					handleIncomingMessage(msg, freshConfig);
				},
				onError: (err) =>
					logger.error({ msg: "iMessage client error", error: err.message }),
			});
